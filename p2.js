// 金色飞贼-OPUS · p2 — POISON build (skill = poison)
// Base = v17 core (star efficiency + predictive intercept fire + cover + BFS star race).
//
// p2 fixes p1's losing pattern (every ranked loss was shots 0/0 -> crashed, twice after a
// wasted poison cast). Ranked review proved poison is a PARTIAL slow, not a full freeze:
...
function onIdle(me, enemy, game) {
  ...
  if (foe && canShoot(pos, foe, map)) {
    ...
    //    NOT used when the enemy is aimed at us (poison is a partial slow, won't deny their
    //    shot -> that's a dodge situation, handled below). Skip shielded/poisoned/0-star mutual.
    if (canPoison && !shielded && !foePoisoned && dir === wantDir && !good &&
        !enemyAimed && dist <= 5 && !insuranceLess) { me.poison(); return; }
