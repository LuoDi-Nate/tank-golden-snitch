// 金色飞贼-OPUS · opv21 —— opv20 + R16技能门控(对cloak/teleport/overload敌无条件避其射击车道)
  ...
  if (target) {
    ...
    if (next) {
      if (isOpen(next, map)) {
        ...
        // R16(技能门控):对"从看不见处/跨车道偷杀"的敌人(cloak可见待隐/teleport会瞬移/overload就绪),
        //   寻路无条件避开它能一枪打到的同行/同列格(与星数无关)。逐盘复盘:5/6败=走进这类敌的射击车道被秒。
        var ambusher = (foeSkillType === "cloak") || (foeSkillType === "teleport") || enemyOverloadReady;
        var nextBad = (foe && !samePos(next, foe) && enemyThreatens(foe, foeDir, next, map)) ||
                      ...
                      (ambusher && foe && !samePos(next, foe) && isAligned(next, foe) &&
                       manhattan(next, foe) <= 8 && canShoot(foe, next, map));  // R16-技能门控
        if (nextBad) {
