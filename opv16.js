// 金色飞贼-OPUS · opv16 —— opv15(存活优先+彩蛋) + R1去星数门控// 金色飞贼-OPUS · opv14 —— opv13 + R16去星数门控(存活优先:数据证明我从不输星,每少死一次=多一胜)主动脱出超载杀伤带
...
function onIdle(me, enemy, game) {
  // 🎭 彩蛋:每5帧随机喷一句骚话(纯嘲讽,无战斗意义)。用全局 speak + try/catch,
  //    喊完继续跑正常决策、每帧仍排一个真实动作,绝不因此 runTime。
  if ((game.frames % 5) === 0) {
  ...
  var avoidOverloadBand = enemyOverloadReady;  // R1去星数门控(存活优先:跨列干净杀无视星数都会输)
  ...
  // ---- 2c. R1-主动:我当前就在"超载就绪敌"的两车道杀伤带里(近距≤5),会被跨行/跨列一枪秒——
  //     主动侧移脱出(与星数无关)。逐帧复盘:多场死于站在超载敌 +垂直车道被跨列打死。
  if (foe && enemyOverloadReady && manhattan(pos, foe) <= 5 && inOverloadBand(pos, foe)) {
    if (stepOffLane(me, dir, pos, foe, map)) return;
  }
    ...
    if (next) {
      if (isOpen(next, map)) {
        ...
        var nextBad = (foe && !samePos(next, foe) && enemyThreatens(foe, foeDir, next, map)) ||
                      ...
                      // R16:抢星途中,别走进"可见敌能一枪打到的同行/同列格"(它会转身+开火,不只当前正瞄)。
                      //      限:有星可抢 + 我不领先 + 敌在射程内(≤8),避免过度被动。逐帧复盘中距走进弹道的头号败因。
                      (star && foe && !samePos(next, foe) &&
                       isAligned(next, foe) && manhattan(next, foe) <= 8 && canShoot(foe, next, map));  // R16去星数门控:存活优先(我从不输星)
        if (nextBad) {
