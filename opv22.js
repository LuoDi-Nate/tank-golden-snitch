// 金色飞贼-OPUS · opv22 —— opv20 + R20抖动走位(看不见敌时抢星途中每7帧插一步垂直,破对手Lead预判反伏击)
  ...
  if (target) {
    ...
    if (next) {
      if (isOpen(next, map)) {
        ...
        // R20 抖动走位:看不见敌人(潜在蹲草/隐身伏击)+抢星途中,每隔几帧插一步垂直移动,
        //   打乱对手对我直线移动的"Lead"预判,躲掉那致命的第一枪。我从不输星,牺牲一点星效率换存活。
        if (!foe && star && (game.frames % 7 === 3)) {
          var toD = directionTo(pos, next);
          var perp = (toD === "left" || toD === "right") ? ["up", "down"] : ["left", "right"];
          for (var jj = 0; jj < 2; jj++) {
            var pn = add(pos, delta(perp[jj]));
            if (isOpen(pn, map)) {
              if (dir === perp[jj]) me.go(); else me.turn(turnDirection(dir, perp[jj]));
              return;
            }
          }
        }
