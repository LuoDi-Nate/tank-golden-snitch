// 金色飞贼-OPUS · opv18 —— opv17 + 进攻性蹲草伏击(lane-lock,学myth冲第一;封顶15帧防runTime)
...
function onIdle(me, enemy, game) {
  ...
  if (game.frames === 0 && G) { G.__nav = null; G.__lf = null; G.__camp = 0; }
  ...
  if (foe && canShoot(pos, foe, map)) {
    ...
    if (danger && dist >= 7 && !haveEdgeShot) { if (escapeMove(me, dir, pos, foe, map)) return; }
  ...
  // ---- 1d. 进攻性蹲草伏击(lane-lock,学 myth):我在草丛=隐身,超载就绪、不落后、手边没近星可抢时,
  //     朝敌人方向预装填并保持隐身,等它走进我的行/列被两车道弹伏杀(上面交战层会开火)。
  //     封顶15帧防空转 runTime;没近星才蹲=不牺牲抢星;离开草丛即清零。
  var onGrass = map[pos[0]] && map[pos[0]][pos[1]] === "o";
  if (!onGrass && G) G.__camp = 0;
  var nearStar = star && manhattan(pos, star) <= 4;
  if (foe && onGrass && (overloaded || canOverload) && myStars >= enemyStars && !nearStar && !incoming) {
    var camp = (G && G.__camp) || 0;
    if (camp < 15) {
      if (G) G.__camp = camp + 1;
      if (canOverload && !overloaded && canFireNow && !shielded) { me.overload(); return; }  // 隐身装填
      var wantF = directionTo(pos, foe);
      if (dir !== wantF) { me.turn(turnDirection(dir, wantF)); return; }                     // 朝敌伏击方向
      var gdirs = ["up", "right", "down", "left"];                                            // 侧移到相邻草格换伏击角(防空转)
      for (var gi = 0; gi < 4; gi++) {
        var gn = add(pos, delta(gdirs[gi]));
        if (isOpen(gn, map) && map[gn[0]] && map[gn[0]][gn[1]] === "o" && !samePos(gn, foe)) {
          moveToward(me, dir, pos, gn); return;
        }
      }
      // 无相邻草格:不强蹲,落到下面正常抢星(避免原地空转)
    } else if (G) { G.__camp = 0; }
  }
