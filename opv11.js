// 金色飞贼-OPUS · opv11 —— opv10 + R15不硬刚远程对狙
// opv11:敌可见+同线+瞄我,但我这枪没优势(没正对/非稳中且未装填)→任意距离脱离共线,不站同线对狙。
// 高压射击只在"敌没瞄我(安全)"时开火。逐帧复盘:多场败于远距同线对狙我miss他命中。
...
function onIdle(me, enemy, game) {
  ...
  if (foe && canShoot(pos, foe, map)) {
    ...
    // 3) R15 不硬刚远程对狙:敌瞄着我(danger)但我这一枪没优势(没正对 或 非稳中且未装填)→
    //    任意距离都脱离共线,别站在同线跟狙击手对射(我miss他命中就送头)。逐帧复盘头号败因。
    //    只有上面"有优势枪"时才开火;这里覆盖了劣势换命(danger含enemyAimed)。
    var haveEdgeShot = canFireNow && dir === wantDir && (good || overloaded);
    if (danger && !haveEdgeShot) { if (escapeMove(me, dir, pos, foe, map)) return; }

    // 4) 高压射击(仅在敌没瞄我=安全时):正对清晰射线就开火,逼其一直躲。
    if (canFireNow && !shielded && !danger && dir === wantDir && !losingMutual) { me.fire(); return; }
