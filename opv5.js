// 金色飞贼-OPUS · opv5 —— 超载 + 两车道预判拦截 + 保优避战 + 提前预装填
// opv5 相对 opv4:已装填时,预判拦截同时算 +垂直那条车道的第二颗子弹(overloadIntercepts),
// 修复对交叉走位的漏打;预装填距离放宽到 ≤6。冲榜前10(RS~2932)的机制升级尝试。
// opv4 相对 opv3:领先2星以上遇威胁时,退向"最远的安全格"主动拉开距离(fleeSafely),而非原地小侧移。
...
function onIdle(me, enemy, game) {
  ...
  // ---- 0. 预判拦截射击(超载=两车道预判)----
  // 若沿当前朝向开火、结合敌人运动会撞上敌人,立即开火。已装填 overload 时,同时模拟「+垂直
  // 那条车道的第二颗子弹」——对手侧移进那条车道也被预判命中(修复超载版对交叉走位的漏打)。
  // 只跳过「劣势换命」:敌也瞄着我且我方星数不占优,双亡会按星数判负。
  if (foe && canFireNow && !shielded) {
    var eAimNow = foeDir === directionTo(foe, pos) && canShoot(foe, pos, map);
    var lm0 = eAimNow && (myStars <= enemyStars);
    var willHit = overloaded ? overloadIntercepts(pos, dir, foe, foeDir, map)
  ...
  // 快节奏对枪里几乎触发不了,只能用单发对轰两车道输掉。改为:敌人靠近(≤6)、没瞄我、也没有
  ...
  if (foe && canOverload && !overloaded && canFireNow && !shielded && !incoming) {
    ...
    if (manhattan(pos, foe) <= 6 && !preAimMe) { me.overload(); return; }
  }
  ...
  if (foe && canShoot(pos, foe, map)) {
    ...
    // 0) 保优避战:领先2星以上时不必冒险对枪。遇到威胁一律主动"脱离接触"——退向最远的安全格
    //    拉开距离,而不是原地小侧移。复盘 Fei-Fei/破:3星领先却纯原地躲、被持续喷射拖到clip死。
    var bigLead = myStars >= enemyStars + 2;
    if (bigLead && danger) { if (fleeSafely(me, dir, pos, foe, map)) return; }
...
// 保优脱离:领先时优先退向"安全(敌无射线)且离敌最远"的相邻格,主动拉开距离脱离接触,
// 避免长时间被持续喷射clip死。找不到就退回普通闪避。
function fleeSafely(me, dir, pos, foe, map) {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    var score = (exposed ? 0 : 1000) + manhattan(n, foe);   // 先要安全,再要更远
...
// ---- 瞄准 ----
// 单条车道的预判拦截:从 start 出发沿 dir 的子弹(2格/帧) vs 敌沿 foeDir 的运动(1格/帧)。
function interceptFrom(start, dir, foe, foeDir, map) {
  ...
  for (var f = 0; f < 12; f++) {
    for (var sub = 0; sub < 2; sub++) {           // 子弹每帧走两格
...
function interceptShot(pos, dir, foe, foeDir, map) {
  return interceptFrom(pos, dir, foe, foeDir, map);
}
// 超载 = 两车道弹的预判:除本车道外,再模拟 +垂直方向偏 1 的第二颗子弹。任一命中即可开火。
function overloadPerp(dir) {
  return (dir === "left" || dir === "right") ? [0, 1] : [1, 0];  // 横向偏 +y,纵向偏 +x
}
