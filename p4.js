// 金色飞贼-OPUS · p4 — POISON build (skill = poison)
// Base = v17 core (star efficiency + predictive intercept fire + cover + BFS star race).
//
// p4 eliminates runTime losses: v17/p3 ran a 600-node BFS (sometimes twice) EVERY frame,
// which over a 128-frame game accumulates past the runtime budget -> auto-loss. p4 CACHES
// the path to the star in globalThis and only recomputes when the star changes or we leave
// the path, so BFS runs a few times per game instead of ~128x. Pathing behaviour is identical.
//
...
function onIdle(me, enemy, game) {
  ...
  // Reset the cached nav path at the start of a match (globalThis persists across frames).
  ...
  // ---- 3. STAR RACE / APPROACH (cached path -> avoids per-frame BFS / runTime) ----
  var target = star || foe;
  if (target) {
    // BFS only for the STATIC star goal (cached). A moving foe uses cheap greedy stepping —
    // full pathfinding to a moving goal every frame was a runTime source on maze maps.
...
// Full shortest path start..goal (inclusive) via bounded BFS with parent pointers, or null.
...
// Cached next-step toward goal. Recomputes the BFS path only when the goal changes or we
// have left the cached path; otherwise it's an O(path) index advance. Crucially it also
// CACHES FAILURE: when no path exists (e.g. a star stranded behind a teleport gap on a maze
// map), it backs off BFS for COOL frames instead of re-running a 600-node search every frame
// — that unbounded retry was the real runTime auto-loss cause. Returns null -> caller wanders.
function planStep(pos, goal, map) {
  var g = (typeof globalThis !== "undefined") ? globalThis : null;
  var gk = goal[0] + "," + goal[1];
  var c = g ? g.__nav : null;

  // Still on a valid cached path for this goal -> cheap index advance.
  if (c && c.gk === gk && c.path) {
    for (var j = c.i; j < c.path.length; j++) {
      if (samePos(c.path[j], pos)) { c.i = j; return c.path[c.i + 1] || null; }
    }
  }
  // Recently failed for this goal -> back off (don't hammer BFS every frame).
