// 金色飞贼-OPUS · opv6 —— 超载版 + 保优避战 + 提前预装填 + 反超载杀伤带(用户tip)
...
function onIdle(me, enemy, game) {
  ...
  // 用户tip:能读到对手技能。对手也是 overload 时,他的两车道弹能跨车道打死我。
  ...
  // ---- 0.6 反超载:对手是 overload 且我不领先星数时,别待在他的两车道杀伤带里(同线或相邻±1
  //     车道的近距)——他一发两车道弹就能跨车道秒我。此时若没有立刻的必中枪(上面已开火),
  //     就主动退出杀伤带、拉开距离,改打星/绕后。这是王者段最大的送头败因(用户tip)。
  if (foe && enemyHasOverload && behindOrEven && inOverloadBand(pos, foe) && !incoming) {
    if (fleeSafely(me, dir, pos, foe, map)) return;
  }
    ...
    if (next) {
      if (isOpen(next, map)) {
        ...
        var nextBad = (foe && !samePos(next, foe) && enemyThreatens(foe, foeDir, next, map)) ||
                      (foe && enemyHasOverload && behindOrEven && !samePos(next, foe) && inOverloadBand(next, foe));
        if (nextBad) {
